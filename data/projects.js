// data/projects.js

export const projects = [
  {
    id: 'portfolio-website',
    title: 'وب‌سایت نمونه‌کار شخصی (همین سایت)',
    shortDescription: 'پورتفولیوی شخصی با طراحی Material Design و پشتیبانی کامل از راست‌چین',
    description: 'همین وب‌سایتی که در حال مشاهده آن هستید؛ یک پورتفولیوی شخصی با Next.js که برای معرفی خودم، نمایش نمونه‌کارها، رزومه و راه ارتباطی طراحی و توسعه دادم. تمرکز اصلی روی طراحی تمیز بر پایه Material Design 3، پشتیبانی کامل از زبان فارسی و راست‌چین (RTL)، و ساختاری قابل نگهداری با جداسازی کامل محتوا از کامپوننت‌ها بوده.',
    category: 'وب',
    images: ['/images/projects/portfolio1.png', '/images/projects/portfolio2.png', '/images/projects/portfolio3.png'],
    technologies: ['Next.js', 'React', 'JavaScript', 'CSS (Material Design 3)'],
    date: '۱۴۰۵',
    duration: 'پروژه در حال توسعه مستمر',
    role: 'طراح و توسعه‌دهنده',
    client: 'پروژه شخصی',
    liveUrl: '[آدرس سایت روی Railway]',
    githubUrl: null,
    features: [
      'صفحات خانه، درباره من، نمونه‌کارها، رزومه و تماس',
      'طراحی مینیمال بر پایه Material Design 3 با تم رنگی اختصاصی',
      'پشتیبانی کامل از فارسی و چیدمان راست‌چین (RTL)',
      'جداسازی محتوا از کامپوننت‌ها برای مدیریت و به‌روزرسانی آسان',
      'فرم تماس با اعتبارسنجی کامل فیلدها',
      'طراحی کاملاً واکنش‌گرا برای موبایل و دسکتاپ'
    ],
    challenges: [
      'پیاده‌سازی صحیح چیدمان راست‌چین در کنار آیکون‌ها و فرم‌ها',
      'طراحی یک سیستم رنگ و تایپوگرافی یکپارچه برای کل سایت'
    ]
  },
  {
    id: 'bakery-shirin',
    title: 'قنادی شیرین',
    shortDescription: 'وب‌سایت فروشگاهی برای یک قنادی؛ نمایش محصولات و سفارش آنلاین',
    description: 'وب‌سایتی برای یک قنادی با Next.js طراحی و توسعه دادم که شامل صفحه اصلی با اسلایدر تبلیغاتی، بخش «داستان ما» برای معرفی قنادی، صفحه لیست محصولات، و صفحه اختصاصی هر محصول با نمایش تخفیف و امکان افزودن به سبد خرید است.',
    category: 'وب',
    images: ['/images/projects/bakery1.png', '/images/projects/bakery2.png', '/images/projects/bakery3.png'],
    technologies: ['Next.js', 'React', 'JavaScript', 'CSS'],
    date: '۱۴۰۵',
    duration: '—',
    role: 'طراح و توسعه‌دهنده',
    client: 'قنادی شیرین',
    liveUrl: null,
    githubUrl: null,
    features: [
      'اسلایدر تبلیغاتی در صفحه اصلی',
      'بخش «داستان ما» برای معرفی هویت و تجربه قنادی',
      'صفحه محصولات با تصاویر، قیمت و برچسب تخفیف',
      'صفحه اختصاصی هر محصول با شمارنده تعداد و دکمه افزودن به سبد خرید',
      'طراحی هماهنگ با هویت بصری گرم قنادی (رنگ طلایی و کرم)'
    ],
    challenges: [
      'هماهنگ کردن رنگ‌بندی و طراحی با حس گرم و سنتی یک قنادی',
      'پیاده‌سازی سبد خرید و شمارنده تعداد محصول'
    ]
  },
  {
    id: 'tools-store',
    title: 'فروشگاه آنلاین ابزار و یراق‌آلات',
    shortDescription: 'فروشگاه اینترنتی ابزارآلات صنعتی و خانگی با پنل مدیریت محصولات',
    description: 'یک فروشگاه اینترنتی برای فروش ابزارآلات صنعتی و خانگی، شامل دسته‌بندی محصولات بر اساس کاربرد، مقایسه محصولات مشابه، و پنل مدیریت ساده برای افزودن و ویرایش محصولات توسط فروشنده.',
    category: 'وب',
    images: ['/images/projects/tools-store1.png', '/images/projects/tools-store2.png', '/images/projects/tools-store3.png'],
    technologies: ['React', 'Next.js', 'Node.js', 'MongoDB', 'Stripe'],
    date: '۱۴۰۵',
    duration: '۲ ماه',
    role: 'طراح و توسعه‌دهنده',
    client: 'فروشگاه ابزار پارس (پروژه نمونه)',
    liveUrl: null,
    githubUrl: null,
    features: [
      'دسته‌بندی محصولات بر اساس نوع ابزار و کاربرد',
      'جستجو و فیلتر بر اساس برند، قیمت و امتیاز',
      'مقایسه چند محصول مشابه در یک صفحه',
      'سبد خرید و پرداخت آنلاین',
      'پنل ساده مدیریت محصولات برای فروشنده'
    ],
    challenges: [
      'طراحی فیلترهای ترکیبی برای تعداد زیاد محصول',
      'ساخت پنل مدیریت ساده و کاربردی برای فروشنده غیرفنی'
    ],
    results: [
      'نمونه‌ای برای نمایش توانایی ساخت فروشگاه‌های اینترنتی کامل'
    ]
  },
];

export const categories = ['همه', 'وب', 'موبایل', 'دسکتاپ'];
